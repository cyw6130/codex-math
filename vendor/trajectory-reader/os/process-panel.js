...
(function () {
  ...
  function mount(app, input) {
    ...
    const add=(text,cls)=>detail.append(element('p',text,cls));
    function show(entry) {
      ...
      if(run){
        ...
        for(const artifact of run.artifacts){
          const item=element('details');item.append(element('summary',kinds[artifact.kind]||'研究材料'));
          ...
          for(const message of artifact.messages||[])item.append(element('p',message.role+'：'+message.content));
          detail.append(item);
        }
