    ...
    function nodeStyle(node) {
      ...
      if (inference || node.t === "i") {
        ...
        return {
          ...
          lineWidth: dim ? 1 : 1.4,
          dash,
          label: dim ? "" : truncate(`${inference?.k ?? "证明"} · ${inference?.p.length ?? 0} 前提`),
      ...
      const isFoundation = foundations.has(node.id);
      const mathState = isFact ? null : bundle.states[node.id];
      ...
      return {
        ...
        lineWidth: dim ? 1 : 1.6,
        dash,
        label: dim ? "" : truncate(entry?.t ?? node.id),
