...
(function publishTrajectoryView(root, factory) {
  ...
  function mount(container, bundle) {
    ...
    const layout = {
      nodes: bundle.nodes.map(node => {
        ...
        return inference || node.t === "i" ? {
          id: node.id, nodeKind: "inference", title: `${KIND_CN[inference?.k] ?? inference?.k ?? "Inference"} · ${inference?.p?.length ?? 0} 个前提`,
    ...
    const mathHTML = text => String(text ?? "").split(/(\$\$[\s\S]*?\$\$|\$[^$\n]*?\$)/gu).map(part => {
      const block = part.startsWith("$$") && part.endsWith("$$") && part.length > 4;
      const inline = !block && part.startsWith("$") && part.endsWith("$") && part.length > 2;
      if (!block && !inline) return esc(part);
      if (!root.katex?.renderToString) return esc(part);
      try { return root.katex.renderToString(part.slice(block ? 2 : 1, block ? -2 : -1), { displayMode: block, throwOnError: false, output: "html" }); }
    ...
    const showStageTitle = id => { if (stageTitle) stageTitle.textContent = objectTitle(id); };
    ...
    function statusText(id) {
      return state.status[id] === "acc" ? "已进入权威 M" : state.status[id] === "draft" ? "在研究草稿图里，尚未结算" : "当前步尚未出现";
    }

    function objectButton(id, title) {
      return `<button class="jump" type="button" data-obj="${esc(id)}">${esc(title)}<small>${esc(id)}</small></button>`;
    }

    function showObject(id, switchTab = true) {
      const entry = entries.get(id);
      const inference = inferences.get(id);
      if ((!entry && !inference) || (currentMode && acceptedOnly && state.status[id] === "draft")) return;
      if (state.mathStates && state.status[id] === "absent") return;
      showStageTitle(id);
      if (inference) {
        q("#paneD").innerHTML = `<div class="sec">Inference · ${esc(inference.k)}</div><h4>${esc(KIND_CN[inference.k] ?? inference.k)}</h4><div class="stmt">${mathHTML(inference.a)}</div><dl class="facts"><dt>id</dt><dd class="mono">${esc(id)}</dd><dt>前提数</dt><dd>${inference.p.length}</dd><dt>当前</dt><dd>${esc(statusText(id))}</dd></dl><div class="sec">全部前提</div>${inference.p.map(parent => objectButton(parent, entries.get(parent)?.t ?? parent)).join("")}<div class="sec">结论</div>${objectButton(inference.c, entries.get(inference.c)?.t ?? inference.c)}`;
      } else {
        const witness = (state.deriv ?? bundle.deriv)[id];
        const basis = witness?.basis === "b0" ? "B₀ · 当前语境直接采用" : witness?.basis === "proof" ? `proof · ${(witness.establishingProofIds ?? []).join(", ")}` : witness?.basis === "negation" ? `否定对 · ${witness.negatingClaimEntryId}` : "尚无 proof 指向它";
        const mathStates = state.mathStates ?? state.states ?? bundle.states;
        const mathStatus = currentMode && state.status[id] === "draft" ? "草稿 · 未审核" : entry.c === "fact" ? "天然可用" : mathStates[id] ?? "open";
        q("#paneD").innerHTML = `<div class="sec">${esc(KIND_CN[entry.k] ?? entry.k)} · ${esc(entry.c)}</div><h4>${esc(entry.t)}</h4><div class="stmt">${mathHTML(entry.s)}</div><dl class="facts"><dt>id</dt><dd class="mono">${esc(id)}</dd><dt>数学状态</dt><dd>${esc(mathStatus)}</dd><dt>建立依据</dt><dd>${esc(entry.fab ? "没有 proof，恒为 open" : basis)}</dd><dt>当前</dt><dd>${esc(statusText(id))}</dd></dl><div class="sec">直接前提</div>${(bundle.prem[id] ?? []).filter(item => !state.mathStates || state.status[item.inf] !== "absent").map(item => objectButton(item.inf, `${KIND_CN[item.k] ?? item.k} ← ${item.n} 个前提`)).join("") || '<p class="none">没有直接前提。</p>'}<div class="sec">直接后续</div>${(bundle.succ[id] ?? []).filter(item => !state.mathStates || state.status[item.inf] !== "absent").map(item => objectButton(item.to, entries.get(item.to)?.t ?? item.to)).join("") || '<p class="none">没有直接后续。</p>'}`;
      }
      selected = id;
